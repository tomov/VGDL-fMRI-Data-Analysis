function neurosynth_rsa(rsa_idx, use_smooth, lateralized, nperms, parcel_idx, subbatch_size, corr_type, dist)
...
if ~exist('parcel_idx', 'var') || isempty(parcel_idx)
    ...
    filename = sprintf('mat/neurosynth_rsa_%d_us=%d_l=%d_nperms=%d_nroi=%d_corr=%s_dist=%s.mat', rsa_idx, use_smooth, lateralized, nperms, length(roi_masks), corr_type, dist);
else
    ...
    filename = sprintf('mat/neurosynth_rsa_%d_us=%d_l=%d_nperms=%d_nroi=%d_pi=%s_corr=%s_dist=%s.mat', rsa_idx, use_smooth, lateralized, nperms, length(roi_masks), tmp{1}, corr_type, dist);
end
filename

% run actual rsa
rsa_helper(EXPT, rsa_idx, roi_masks, filename, nperms, subbatch_size, region, which, corr_type, dist);
