...
filename = sprintf('mat/neurosynth_rsa_HRR_groundtruth_avg_us=%d_nperms=%d_dist=%s_%s.mat', use_smooth, nperms, dist, what);
...
filename = sprintf('mat/searchlight_rsa_HRR_groundtruth_avg_us=%d_r=%.4f_nperms=%d_dist=%s_%s.mat', use_smooth, radius, nperms, dist, what);
...
for r = 1:length(ROI)
    ...
    ROI(r).masked_voxel_idx = find(roi_mask(whole_brain_mask));
    r
end
