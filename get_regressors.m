function [regs, X, fields] = get_regressors(subj_id, run, conn, do_cache, regressors_collection)
    ...
    if ~exist('regressors_collection', 'var')
        regressors_collection = 'regressors';
        filename = fullfile(get_mat_dir(false), 'get_regressors', sprintf('get_regressors_subj%d_run%d.mat', subj_id, run.run_id));
    else
        filename = fullfile(get_mat_dir(false), 'get_regressors', sprintf('get_regressors_subj%d_run%d_c=%s.mat', subj_id, run.run_id, regressors_collection));
    end
    filename
    ...
    switch regressors_collection
        case {'empa_regressors'}
            ...
            plays_post_collection = 'empa_plays_post3'; % separate collection
            ...
            % empa_plays_post3
            post_fields = [post_fields {'hypothesized_terminations_len', 'hypothesized_terminations_flag', 'falsified_terminations_len', 'falsified_terminations_flag', 'win_plan_length', 'avg_plan_length', 'num_plans', 'win_plan_ac', 'avg_plan_ac', 'win_plan_eff', 'avg_plan_eff'}];
