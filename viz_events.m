...
for r = 1:length(runs)
    ...
    offs = 1;
    st = round(offs * 1000) ;
    en = round((run.end_time - run.scan_start_time + offs) * 1000);
    rr(st:en) = 8;
    ...
    for b = 1:length(blocks)
        ...
        st = round((block.start_time - run.scan_start_time + offs) * 1000) + 3;
        en = round((block.end_time - run.scan_start_time + offs) * 1000) - 3;
        bb(st:en) = 6;
        ...
        for i = 1:length(instances)
            ...
            st = round((instance.start_time - run.scan_start_time + offs) * 1000) + 5;
            en = round((instance.end_time - run.scan_start_time + offs) * 1000) - 5;
            ii(st:en) = 4;
            ...
            for p = 1:length(plays)
                ...
                st = round((play.start_time - run.scan_start_time + offs) * 1000) + 7;
                en = round((play.end_time - run.scan_start_time + offs) * 1000) - 7;
                pp(st:en) = 2;
                if isempty(play.win)
                    win = 'T';
                elseif play.win
