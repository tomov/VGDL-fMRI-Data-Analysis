% load HRR kernel
%
function [ker, features] = load_HRR_kernel(subj_id, which_run_ids, what, normalize, concat, novelty)
    if concat == 0 && novelty == 1
        ...
        load(filename, 'theory_kernel', 'sprite_kernel', 'interaction_kernel', 'termination_kernel', 'r_id', 'theory_Xx', 'sprite_Xx', 'interaction_Xx', 'termination_Xx');
    else
        % new stuff
