...
for subj in ${subjects[*]}; do
    outfileprefix="output/fit_gp_CV_noHRF_${subj}_${use_smooth}_${glmodel}"
    echo ---------------------------------------------------------------------------------
    echo Subject ${subj}, file prefix = $outfileprefix

    # send the job to NCF
    #
    sbatch_output=`sbatch -p ncf --mem 20001 -t 0-3:20 -o ${outfileprefix}_%j.out -e ${outfileprefix}_%j.err --wrap="matlab -nodisplay -nosplash -nojvm -r $'fit_gp_CV_noHRF(${subj}, ${use_smooth}, ${glmodel}, \'${mask}\', ${subsample_only});exit'"`
    # for local testing
    ...
    echo fit_gp_CV_noHRF.sh for subject ${subj}: ${outfileprefix}_${job_id}.out -- $sbatch_output >> jobs.txt
